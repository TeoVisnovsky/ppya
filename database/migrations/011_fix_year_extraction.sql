-- Fix year extraction to properly extract and convert to integer
...
BEGIN
  ...
  -- Extract year after "vyroby:" with robust digit extraction
  ...
  IF v_rok_pos > 0 THEN
    ...
    v_year_extracted := '';
    FOR i IN 1..LENGTH(v_substring) LOOP
      IF SUBSTRING(v_substring FROM i FOR 1) ~ '^\d$' THEN
        v_year_extracted := v_year_extracted || SUBSTRING(v_substring FROM i FOR 1);
      ELSE
        EXIT;
      END IF;
    END LOOP;
    
    -- Check if we got 4 digits
    IF LENGTH(v_year_extracted) = 4 THEN
      v_year_int := v_year_extracted::INT;
      -- Validate year is in reasonable range
      IF v_year_int >= 1900 AND v_year_int <= 2030 THEN
        v_year := v_year_extracted;
      END IF;
    END IF;
  END IF;
